                ...
                {displayIssues.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {displayIssues.map((issue, index) => (
                            <div
                                key={issue._id}
                                className="bg-base-200 rounded-xl overflow-hidden shadow-lg border border-base-300 hover:shadow-xl transition-shadow duration-300 group cursor-pointer"
                            ...
                            >
                                ...
                                <div className="p-5">
                                    ...
                                    <h3 className="font-['Satoshi'] text-lg font-bold text-base-content mb-2 line-clamp-2">
                                        {issue.title}
                                    </h3>

                                    {/* Meta Info */}
                                    <div className="space-y-1 text-sm text-base-content/70 font-['Satoshi']">
                                        <div className="flex items-center gap-2">
                                            <FiTag className="text-base-content/40" size={16} />
                                            <span>{issue.category}</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <FiMapPin className="text-base-content/40" size={16} />
                                            <span className="truncate">{issue.location}</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <FiClock className="text-base-content/40" size={16} />
                                            <span>
                                    ...
                                    {(issue.upvotes > 0 || issue.upvotedBy?.length > 0) && (
                                        <div className="mt-3">
                                            <button
                                                className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary/10 hover:bg-primary/20 text-primary rounded-lg font-['Satoshi'] text-sm font-medium transition-colors"
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    navigate(`/issue/${issue._id}`);
                                                }}
                                            >
                                                <FiThumbsUp className="fill-primary" size={16} />
                                                <span>
                                    ...
                                    <div
                                        className="mt-4 inline-flex items-center font-['Satoshi'] text-primary font-semibold text-sm hover:text-primary transition-colors"
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            navigate(`/issue/${issue._id}`);
                                        }}
